export class SceneDrawing {
  ...
  constructor(sceneElement) {
    ...
    this.timerElem =
      this.sceneElement.getElementsByClassName("sceneTimerProgress")[0];
    this.timerText = this.sceneElement.getElementsByClassName(
      "sceneTimerCurrentTime"
    )[0];

    this.imageElement = this.sceneElement.getElementsByClassName(
      "sceneDrawingMainImage"
    )[0];

    this.canvasElement =
      this.sceneElement.getElementsByClassName("sceneDrawingCanvas")[0];
    this.toolbarElement = this.sceneElement.getElementsByClassName(
      "sceneDrawingToolbar"
    )[0];
    ...
    this.toolbarClass = new Toolbar(this.toolbarElement, this.toolbar);
    this.canvasDrawing = new CanvasDrawing(this.canvasElement, this.toolbar);
  }

  async displayData(data) {
    this.timerElem.style.width =
      (data.timeRemaining / data.maxTime) * 100 + "%";

    window.animator.rotateIncrease =
      (1 - data.timeRemaining / data.maxTime) * 0.75;

    this.timerText.innerText = Math.round(data.timeRemaining) + "s";

    //UNCOMMENT THIS YOU MORON
    let path = "/image/static/" + data.currentImage + ".png";
    //this.imageElement.src = "image/static/" + data.currentImage + ".png";
    //let path = data.currentImage;
    if (
      !(
        this.imageElement.src && new URL(this.imageElement.src).pathname == path
      )
    ) {
      this.imageElement.src = path;
      this.toolbarClass.setColors(await getImageColors(path));
    }
  }

  show() {
    this.sceneElement.style.display = "block";
  }

  hide() {
    this.sceneElement.style.display = "none";
  }
}
...
class Toolbar {
  constructor(toolbarElement, toolbarObject) {
    this.toolbarObject = toolbarObject;
    this.toolbarElement = toolbarElement;
    this.pencilElement = this.toolbarElement.getElementsByClassName(
      "sceneDrawingToolbarToolPencil"
    )[0];
    this.bucketElement = this.toolbarElement.getElementsByClassName(
      "sceneDrawingToolbarToolBucket"
    )[0];
    this.eraserElement = this.toolbarElement.getElementsByClassName(
      "sceneDrawingToolbarToolEraser"
    )[0];
    this.colorsElement = this.toolbarElement.getElementsByClassName(
      "sceneDrawingToolbarColors"
    )[0];

    this.pencilElement.addEventListener("click", () => {
      this.bucketElement.classList.remove("sceneDrawingToolbarToolSelected");
      this.pencilElement.classList.add("sceneDrawingToolbarToolSelected");
      this.eraserElement.classList.remove("sceneDrawingToolbarToolSelected");
      this.toolbarObject.tool = "draw";
    });

    this.bucketElement.addEventListener("click", () => {
      this.bucketElement.classList.add("sceneDrawingToolbarToolSelected");
      this.pencilElement.classList.remove("sceneDrawingToolbarToolSelected");
      this.eraserElement.classList.remove("sceneDrawingToolbarToolSelected");
      this.toolbarObject.tool = "fill";
    });

    this.eraserElement.addEventListener("click", () => {
      this.bucketElement.classList.remove("sceneDrawingToolbarToolSelected");
      this.pencilElement.classList.remove("sceneDrawingToolbarToolSelected");
      this.eraserElement.classList.add("sceneDrawingToolbarToolSelected");
      this.toolbarObject.tool = "eraser";
    });
  }
  ...
  setColors(colors) {
    ...
    for (let color of colors) {
      ...
      clr.classList.add("sceneDrawingToolbarColor");
      clr.style.backgroundColor = color;
      clr.addEventListener("click", () => {
        this.toolbarObject.color = color;
        for (
          let childIndex = 0;
          childIndex < this.colorsElement.children.length;
          childIndex++
        ) {
          this.colorsElement.children[childIndex].classList.remove(
            "sceneDrawingToolbarColorSelected"
          );
        }
        clr.classList.add("sceneDrawingToolbarColorSelected");
      });
      this.colorsElement.appendChild(clr);
    }
    this.colorsElement.children[0].click();
  }
}

const getImageColors = async (path) => {
  let image = new Image();
  image.src = path;
  ...
  for (let i = 0; i < imageData.length; i += 4) {
    ...
    let color = `rgba(${imageData[i]}, ${imageData[i + 1]}, ${
      imageData[i + 2]
    }, 1)`;
